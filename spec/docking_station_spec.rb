...
describe DockingStation do 
    ...
    it 'exception is raised to not release bikes' do
        expect{subject.release_bike(station)}.to raise_error(RuntimeError, "Dock is empty, no bikes to release")
    end
    ...
    it 'working bike should be released' do
        ...
        subject.release_bike(station, bike)
    ...
    it 'fail should be raised to stop broken bikes being released'do
        subject.dock broken_bike
        expect{subject.release_bike(station, broken_bike)}.to raise_error("Sorry this bike is broken and cannot be released")
    end
end